...
function renderGalleryGrid(container, list) {
  ...
  list.forEach(g => {
    ...
    card.innerHTML = `
      <img src="${g.coverImage}" alt="${g.name}" style="width: 100%; height: 100%; object-fit: cover; transition: transform 0.6s ease;" class="card-bg-img" />
      <div style="position: absolute; inset: 0; background: linear-gradient(to top, rgba(15,3,8,0.92) 0%, rgba(15,3,8,0.4) 50%, transparent 100%); display: flex; flexDirection: column; justify-content: flex-end; padding: 1.75rem; pointer-events: none;">
        <h3 style="color: var(--white); font-weight: 800; font-size: 1.25rem; margin-bottom: 0.5rem; text-shadow: 0 2px 10px rgba(0,0,0,0.5);">${g.name}</h3>
        ${g.description ? `
          <div style="background: rgba(255,255,255,0.04); backdrop-filter: blur(8px); border-radius: 10px; padding: 0.6rem 0.85rem; border: 1px solid rgba(255,255,255,0.1); margin-bottom: 0.75rem;">
            <p style="color: rgba(255,255,255,0.8); font-size: 0.82rem; line-height: 1.4; display: -webkit-box; -webkit-line-clamp: 2; -webkit-box-orient: vertical; overflow: hidden;">${g.description}</p>
          </div>
        ` : ''}
...
function openLightbox(gallery) {
  ...
  const imagesHtml = (gallery.images || []).map((imgUrl, idx) => `
    <div style="border-radius: 16px; overflow: hidden; box-shadow: 0 25px 60px rgba(0,0,0,0.5); border: 1px solid rgba(255,255,255,0.08); background: rgba(255,255,255,0.02);">
      <img src="${imgUrl}" alt="${gallery.name} ${idx + 1}" style="width: 100%; height: auto; display: block; transition: transform 0.4s ease;"
  ...
  overlay.innerHTML = `
    ...
    <div style="padding: 7rem 2rem 3rem; text-align: center; background: linear-gradient(to bottom, rgba(201,168,76,0.06) 0%, transparent 100%); border-bottom: 1px solid rgba(255,255,255,0.05); margin-bottom: 3rem;">
      <div style="max-width: 800px; margin: 0 auto;">
        ...
        <h2 style="color: white; font-weight: 800; font-size: clamp(1.8rem, 4vw, 2.6rem); margin-bottom: 1rem;">${gallery.name}</h2>
        ...
        ${gallery.description ? `
          <p style="color: rgba(255,255,255,0.85); font-size: 1.1rem; line-height: 1.7; font-style: italic; position: relative;">
            &ldquo;${gallery.description}&rdquo;
          </p>
        ` : ''}
      </div>
    </div>
